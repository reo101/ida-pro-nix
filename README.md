...
Plugin definitions are selected like Python packages with `plugins = ps: [ ... ];`. Use `plugins = ps: ps.allPlugins;` to install every plugin in the scope. The `ps` scope is extensible with `programs.ida-pro.pluginPackageExtensions`; it also exposes `ida-pro-version` and `ida-sdk` for plugins that need them.
